...
#dataset_name = './datasets/kitti/kitti2011_09_26_drive0001_frame'
dataset_name = './datasets/intel'
...
for framei in range(2):
    ...
    for segi in range(len(cell_max_min_segments)):
        ...
        plt.scatter(Xq[:, 0], Xq[:, 1], c=yq, cmap='jet', s=5, vmin=0, vmax=1, edgecolors=['none'])
    plt.colorbar()
    plt.xlim([-80,80]); plt.ylim([-80,80])
    plt.title(f"{dataset_name.split('/')[-1]}_{framei}")
    plt.savefig(os.path.abspath(f"./outputs/{dataset_name.split('/')[-1]}_{framei}.png"))
